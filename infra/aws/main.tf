...
resource "aws_s3_bucket" "filings" {
  bucket_prefix = "${local.name}-filings-"
...
resource "aws_security_group" "alb" {
  name        = "${local.name}-alb"
  description = "Allow HTTP ingress to Fintel ALB"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "data" {
  ...
  description = "Allow ECS tasks to reach data services"
  vpc_id      = aws_vpc.main.id
...
resource "aws_iam_role_policy" "s3_access" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "${aws_s3_bucket.filings.arn}/*"
    }]
  })
}
